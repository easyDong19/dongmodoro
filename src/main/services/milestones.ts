...
export type MilestoneCardItem = MilestoneRow & {
   ...
   */
  rollup: { spentPomos: number; plannedPomos: number } | null
}
...
export function monthMilestones(uow: UnitOfWork, month: string): MonthMilestones {
  ...
  return uow.run((repos) => {
    ...
    const rollups =
      rollupWeek === null
        ? new Map<string, { spentPomos: number; plannedPomos: number }>()
        : new Map(
            repos.milestones
              .rollup(month, rollupWeek)
              .map((r) => [
                r.milestoneId,
                { spentPomos: r.spentPomos, plannedPomos: r.plannedPomos }
              ])
          )
