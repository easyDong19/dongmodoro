...
/**
 * 2택 한 행 (ADR-031 §1). 배지는 **저장값이 아니라 파생**이라 여기서 붙인다.
 *
 * **남은 몫(`remaining`)이 없다.** est 가 사라져 `max(0, est − 소진)` 의 피감수가
 * 없어졌고, 시간으로 대체하지 않는다 — 이월 항목의 측정 시간은 정의상 0 이다.
 */
...
export type ReviewPending =
  ...
  | {
      ...
      summary: {
        ...
        /** 그 주의 측정 시간(초). 화면이 포맷한다 — main 은 초만 내려보낸다. */
        lastStudiedMeasuredSec: number | null
      }
...
//
// **예외 종류도 하나뿐이다** (ADR-031 §1) — `carry_reduced` 는 줄일 대상인 est 와 함께
// 죽었고, 그와 함께 클램프도 사라졌다.
export type SettleException = { kind: 'drop'; itemId: string }
...
export type SettleResult = {
  ...
  autoCarried: { sourceItemId: string; newItemId: string; title: string }[]
...
export type SettleDecisions = {
  drops: string[]
  /**
   * 예외로 지정되지 않은 항목 전부. 2택에서는 **모든 이월이 여기 온다** — 이월은 더
   * 이상 예외로 전송되지 않기 때문이다. "화면이 몰랐던 이월"을 가려내는 일은 화면이
   * 자기가 그리던 목록을 빼는 방식으로 한다 (R30, ReviewPanel `toastFor`).
   */
  carries: { sourceId: string; title: string }[]
 ...
 * 그래서 모르는 항목은 무시한다.
 *
 * **클램프가 없다** (ADR-031 §1) — 자를 숫자가 페이로드에 없다. 예외는 `drop` 뿐이고
 * 예외 없는 항목은 전부 이월이다.
 */
export function resolveDecisions(
  ...
  const known = new Set(pending.map((row) => row.id))
  const dropped = new Set(exceptions.filter((e) => known.has(e.itemId)).map((e) => e.itemId))
  ...
  for (const row of pending) {
    ...
    decisions.carries.push({ sourceId: row.id, title: row.title })
  }
 ...
 *   4~5. 폐기 · 이월 생성 + 조각 재부모화 (리포지토리 한 번). **주별 행 갱신은 없다** —
 *      박제하던 값이 전부 폐기된 통화다 (ADR-030 §4).
 *   6. 워터마크 전진 — **마지막**이라 앞에서 실패하면 같은 범위가 다음 실행에 다시 잡힌다.
 */
export function settle(
  ...
  return uow.run((repos) => {
    ...
    const { carried } = repos.review.applySettlement({
      ...
      carries: decisions.carries.map((c) => ({ sourceId: c.sourceId })),
      at: now.instant
    })
    ...
    return {
      ...
      autoCarried: decisions.carries.map((c) => ({
