...
export function createWindow(
  ...
  const win = new BrowserWindow({
    ...
    minWidth: BP_MEDIUM,
    /**
     * 세로 하한. 타이머 다이얼의 하한(≈220px)을 **지켜주는 장치** 다
     * (design-system ADR-012 §5). 다이얼은 세로가 모자라면 줄어들지만 하한에서 멈추므로,
     * 이 값이 없으면 그 아래부터 줄어드는 것이 다이얼이 아니라 **아래 컨트롤** 이 된다 —
     * 조절 칩·버튼 줄·캡처 바가 카드의 `overflow-hidden` 에 잘린다.
     *
     * 역산값이다: 다이얼 220 + 다른 줄 합계 ≈290 = 카드 안쪽 ≈510, 여기에 카드 패딩 32 ·
     * main 패딩 48 · 타이틀바 38 을 더하면 ≈628 이고 여유를 두어 640 으로 올렸다.
     * `minWidth` 와 달리 대응하는 브레이크포인트가 없어 상수다 — 낮은 창 레이아웃을
     * 만드는 날 이 값을 그때의 하한으로 내린다.
     */
    minHeight: 640,
    /**
