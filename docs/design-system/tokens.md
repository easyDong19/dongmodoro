...
| `--text-display` | `5.6rem` | 56px | 타이머 숫자의 **하한** (`--font-mono`) — 렌더 크기는 다이얼 한 변에서 파생된다 (ADR-012 §3) | 다이얼이 가장 작을 때의 값이라, 이 토큰을 직접 font-size 로 주는 자리는 이제 없다 |
...
- **타이머 다이얼의 크기 기준은 글자가 아니라 다이얼 한 변이다** ([ADR-012](./decisions/adr-012-fluid-timer-dial.md)).
  ...
  비례하고, 링 지름·눈금 굵기·숫자 크기가 모두 거기서 파생된다. 숫자는 `25.45cqmin`
  (= 56/220)이라 하한에서 정확히 `--text-display` 다 — **새 단계가 아니라 파생값** 이므로
  위 7단계 규칙은 그대로다. ADR-007 §3 이 정한 반대 방향(글자 → 링, `4.2em`)은 이것으로
...
- 토큰화가 끝난 범주: 색(§1) · 폰트(§2) · radius(§3) · 브레이크포인트(§4) · 유리 표면(§5) ·
  모션(§6) · 레이어(§7) · 인터랙션(§8) · 타이머 다이얼(§9 블록, ADR-012). **문서 없는 토큰을 코드에 먼저 만들지 않는다.**
