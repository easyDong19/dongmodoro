...
| `--amber` | `#f0b671` | 5.14:1 | `#8f550f` | 4.61:1 | 부 액센트. 휴식 모드·캘린더의 `오늘` 숫자와 진한 점·배경 광원·타이머 링 그라디언트의 끝. 텍스트 가능 |
...
- 타이머 링 그라디언트는 `--teal` → `--amber` 조합으로만 만든다. 별도 그라디언트용 색을
  추가하지 않는다.
- **`--amber` 의 용도가 2.0.0 에서 줄었다.** 원래 첫 용처는 `예산 초과(+N 배지)` 였으나
  예산·과적이 폐기되면서 초과라는 상태가 사라졌다
  ([ADR-030](../architecture/decisions/adr-030-time-as-progress-currency.md),
  [principles.md §1](./principles.md)). **토큰은 삭제하지 않는다** — 위 표의 남은 용처가
  전부 실사용 중이다. 다만 `--amber` 를 **"많다·넘었다"는 뜻으로 다시 쓰지 않는다.**
  측정 시간에는 비교 대상이 없으므로 그 의미를 만들 근거가 없다.
...
| `--text-sm` | `1.2rem` | 12px | 리스트 항목 제목·행 본문, 안내 문장 | 조작 대역의 주력. 메타(xs)와 1.2 배율 간격 |
| `--text-md` | `1.3rem` | 13px | 기본 본문(다이얼로그·노트), **섹션 제목(+600)** | 같은 크기 더미에서 굵기 혼자 섹션을 가르게 하지 않는다 |
| `--text-lg` | `1.4rem` | 14px | 카드 제목, 강조 본문 | 항상 600 과 함께 — md(400)와의 1px 인접은 굵기가 가른다 |
| `--text-xl` | `1.7rem` | 17px | 화면 단위 제목, 내로우 축약형 타이머 숫자(+600+mono) | 축약형 숫자는 이름 있는 셀렉터 하나로 |
| `--text-display` | `5.6rem` | 56px | 다이얼 하한·상한(`--timer-dial-min`·`--timer-dial-max`)을 `em` 으로 푸는 **기준값** — 타이머 숫자 자체는 다이얼 한 변의 22% 다 (ADR-013 §2) | 이 토큰이 그대로 렌더되는 자리는 없다. "타이머 숫자는 56px" 로 읽지 않는다 — 하한에서도 ≈49px 이다 |
...
- **rem 은 폰트 크기에만 쓴다.** radius·보더·`--target-min`·브레이크포인트는 px 다 —
  ...
  될 수 없다 (ADR-007 §1).
- **타이머 다이얼의 크기 기준은 글자가 아니라 다이얼 한 변이다** ([ADR-012](./decisions/adr-012-fluid-timer-dial.md)).
  한 변이 `clamp(--timer-dial-min, min(100cqw, 100cqh), --timer-dial-max)` 로 칸 크기에
  비례하고, 링 지름·눈금 굵기·숫자 크기가 모두 거기서 파생된다. 숫자는 `22cqmin`
  ([ADR-013](./decisions/adr-013-lighter-timer-dial.md) §2, 하한에서 ≈49px · 상한 320px 에서
  ≈70px) — **새 단계가 아니라 파생값** 이므로 위 7단계 규칙은 그대로다. ADR-007 §3 이 정한 반대 방향(글자 → 링, `4.2em`)은 이것으로
  대체됐고, 두 토큰이 `em` 인 덕에 루트 확대가 다이얼에 닿는다는 §3 의 목적은 남는다.
...
| `--weight-bold` | `700` | 최상위 강조 |
| `--weight-light` | `300` | **타이머 숫자 전용** ([ADR-013](./decisions/adr-013-lighter-timer-dial.md) §3) |

> 시안의 300(light)·500(medium)은 위 3단계로 흡수한다. 3단계 외 굵기 추가 금지 —
> **예외는 `--weight-light` 하나다.** 화면에서 가장 큰 글자의 무게를 덜기 위한 것이라
> 본문·제목에 쓰지 않는다. "여기도 300 이던데" 를 다른 굵기의 근거로 쓰지 않는다.
...
- 장식성 무한 애니메이션 금지는 그대로다. (2.0.0 이전 이 줄이 예로 들던 "초과 글로우"는
  예산 폐기와 함께 사라졌다 — 규칙 자체는 배경 광원·별에 그대로 적용된다.)
...
| `--layer-sticky` | `10` | 내로우 축약형 고정 바 (카드 하단 고정 게이지는 2.0.0 에서 사라졌다 — 남은 소비자는 축약형 바 하나다) |
...
:root {
  ...
  --weight-semibold: 600;
  --weight-bold: 700;
  --weight-light: 300;   /* 타이머 숫자 전용 — ADR-013 §3 */
  ...
  /* 타이머 다이얼 (ADR-012 · ADR-013) — 길이인데 em 인 유일한 예외다. --text-display 에 대해
     해석되므로 루트 확대가 하한·상한을 함께 민다 (ADR-007 §3 이 지키려던 성질). */
  --timer-dial-min: 3.93em; /* ≈220px — ADR-011 §1 이 미디엄 하한에 쓴 값 */
  --timer-dial-max: 5.71em; /* ≈320px — ADR-013 §1: 400 → 320 */
...
- **spacing 은 아직 토큰화하지 않았다.** 시안 실측값이 2·3·4·5·6·7·8·9·10·12·14·16·20px
  ...
  짤 때 결정한다** — 그때 [ADR-004](./decisions/adr-004-focus-and-target.md) 의
  `--target-min` 을 제약으로 함께 본다.
  > **2026-08-06 갱신:** 원래 "M1 Task 7 에서 결정한다"고 적었으나 그 태스크에서 **결정하지
  > 않았다.** Task 7 이 만든 화면은 플레이스홀더 한 줄과 버튼 하나여서 실측할 레이아웃이
  > 없었고, 근거 없이 단계를 뽑는 것은 이 항목이 경고한 "발명"에 해당한다. 그때까지
  > spacing 은 Tailwind 기본 스케일(4px 배수)로 쓰고, **첫 실제 화면을 짤 때** 그 화면의
  > 실측값을 근거로 ADR 을 쓴다.
- 토큰화가 끝난 범주: 색(§1) · 폰트(§2) · radius(§3) · 브레이크포인트(§4) · 유리 표면(§5) ·
  모션(§6) · 레이어(§7) · 인터랙션(§8) · 타이머 다이얼(§9 블록, ADR-012 · ADR-013). **문서 없는 토큰을 코드에 먼저 만들지 않는다.**
...
- **단위를 바꾸지 않는다**: 폰트 크기만 rem, 그 밖은 px. 특히 `--target-min` 을 rem 으로
  ...
  - **예외는 `--timer-dial-min`·`--timer-dial-max` 둘뿐이다** — 길이지만 `em` 이다. 이유는
    [ADR-012 §2](./decisions/adr-012-fluid-timer-dial.md) 에 있고, **"여기도 em 이던데"를
    다른 길이 토큰의 근거로 쓰지 않는다.**
