...
└── plans/           # 진행 중인 구현 계획서·설계 스펙 (일회용, YYYY-MM-DD-<slug>.md)
```

## `docs/plans/` — 구현 계획서 (일회용)

- 구현 작업의 태스크 분해·순서·검증 방법을 담는 **작업 지시서**다. 기획(무엇을)이 아니라
  실행(어떤 순서로)을 기록하며, 파일명은 `YYYY-MM-DD-<slug>.md`. 구현 전 설계 스펙
  (`*-design.md`)도 같은 폴더·같은 규칙을 따른다.
- 계획은 결정을 만들지 않는다 — 설계 결정이 필요하면 ADR 로 architecture 에 먼저 남기고
  계획은 그것을 참조만 한다. 기능 문서·ADR 과 충돌하면 그쪽이 이긴다.
- **이 폴더에는 "아직 끝나지 않은 계획"만 남는다.** 끝난 계획은 체크박스와 옛 모델을 담은 채
  현재 지시처럼 읽히므로 남겨 두지 않는다. 이력은 git 과 PR 이 보존한다.
- **스킬의 기본 경로(`docs/superpowers/` 등)에 계획·스펙을 만들지 않는다.** 경로를 이 폴더로
  바꿔 저장한다.

### 계획을 끝낼 때 — 구현 PR 의 마지막 태스크

계획서·설계 스펙은 **그것을 구현한 PR 안에서** 삭제한다. 삭제 전에 아래를 먼저 옮긴다:

1. **"왜"는 소유 문서로.** 기각된 선택지·트레이드오프·불변식처럼 계획에만 있는 근거는
   ADR(`architecture/decisions/`·`design-system/decisions/`)이나 기능 문서로 옮긴다.
   규칙대로라면 원래 거기 있어야 할 내용이다.
2. **미룬 범위는 `docs/tmp/` 로.** "이번에 뺀 것"·후속 작업 중 아직 유효한 것은 항목별
   tmp 메모로 옮긴다. 계획과 함께 지우면 백로그가 사라진다.
3. **들어오는 링크는 permalink 로.** 이 파일을 가리키는 링크를 삭제 직전 커밋의 GitHub
   permalink(`https://github.com/easyDong19/dongmodoro/blob/<sha>/docs/plans/...`)로 바꾼다.
   `docs/decision-log/` 도 링크만 바꾸고 본문은 건드리지 않는다.
...
상태는 **문서가 코드를 얼마나 믿을 만하게 기술하는가**를 말한다. overview 의 `현재 상태`,
meta.yaml 의 `status`, [features/README.md](features/README.md) 의 표가 **같은 값**이어야 한다.

| 상태 | meta.yaml | 뜻 | 다음 상태로 가는 조건 |
|---|---|---|---|
| Draft | `draft` | 구현 전. 문서가 앞으로 만들 것을 기술한다 | 기능이 main 에 머지됨 |
| In Review | `in-review` | 구현됐지만 스펙과 코드의 대조가 끝나지 않았다. 문서와 코드가 어긋날 수 있다 | 스펙의 요구사항·카피·상태를 코드와 대조해 어긋남을 모두 고침 |
| Published | `published` | 구현됐고 대조도 끝났다. 문서가 코드를 기술한다 | — |

- **Published 기능의 UI 문구·동작을 바꾸는 PR 은 스펙도 같은 PR 에서 고친다.** 고치지 못하면
  그 기능을 In Review 로 내린다 — 조용히 어긋난 Published 는 없다.
- 폐기된 요구사항(취소선 + **폐기**)과 미구현 요구사항(`> ⚠️` 블록으로 표시)은 대조를 막지
  않는다. "미구현"이라고 적혀 있으면 그것이 코드와 일치하는 기술이다.
