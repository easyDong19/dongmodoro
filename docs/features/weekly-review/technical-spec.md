...
남는 에러가 하나뿐인 것은 의도된 결과다. 리뷰 확정이 실패할 수 있는 유일한 정상
...
- `review.settle` 입력에 `baseline` 이 **없다.** 리뷰 확정 트랜잭션은 길이를 쓰지 않는다.
- 이미 스냅샷이 박제된 주의 값은 어떤 편집으로도 변하지 않는다. 진행 중인 주에
  스냅샷이 없으면 그 주 첫 세션 기록 시점의 값으로 박제된다 (ADR-013 §2 — sessions
  기록 경로 소관).
- 리뷰 화면의 **진입점은 유지**한다 (UX 프레이밍). 명령 이름·유효 범위·기본값은
  pomo-baseline 소관이며, 이 문서는 "리뷰가 그 명령을 호출할 뿐 트랜잭션을 공유하지
  않는다"만 고정한다.
...
- **이미 존재하는 `weeks` 행의 스냅샷 컬럼은 절대 덮어쓰지 않는다.** 확정이 건드리는
  것은 `settled_at` 뿐이다. 이 규칙이 "지각 리뷰가 진행 중인 주의 단위를 바꾼다"는
...
- 이 기능이 **쓰지 않는** 컬럼: `weeks.planned_at` (week-plan 소관),
  ...
  (길이 변경은 독립 명령 — pomo-baseline), `sessions.*` (리뷰는 세션을 읽기만 한다).
...
| 5 | **3주 만에 복귀** | W36 화 | W32 | W36 | {W33,W34,W35} | 리뷰 화면 **1개**, 항목만 3주분. 전부 이월이 기본. 항목·세션 0인 빈 주는 목록에 안 나옴 |
...
| 12 | **첫 실행 = 계획일** (새 DB, 일요일) | W36 일 | 없음 → **W36** 로 초기화 | W37 | 빈 범위 | 배너 없음(헛배너 방지 성립). **부작용: 설치일이 속한 주(W36)가 즉시 워터마크 뒤로 가서 그 주 기록은 어떤 리뷰에도 나오지 않는다.** 캘린더 점·항목 소진·주간 카드에는 정상 반영. **명시 수용** (PRD R39) |
...
| pomo-baseline | 리뷰는 길이 변경의 **진입점**일 뿐이며, 확정 트랜잭션은 길이를 쓰지 않는다. 이미 스냅샷이 있는 주의 값을 리뷰가 덮어쓰는 경로는 없다 (ADR-013 §3) |
