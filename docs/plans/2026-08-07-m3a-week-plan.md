...
- [x] **Step 4: 코어 루프 수동 검증 체크리스트**
  - [x] 계획 0개 상태에서 타이머·오늘 목록이 여전히 전부 동작한다 (원칙 1)
  - [x] 플래너 확정 → 일반 뷰 → `+ 오늘로` → 재생 → 완료 → **항목 도트와 주간 게이지가 함께** 오른다
  - [x] 조각 0개 항목의 `+ 오늘로` 가 드로어를 연다
  - [x] 자식 조각을 전부 완료하면 완료 제안이 뜨고, `완료로 표시` 를 눌러야 완료된다. 무시하면 active 로 남는다
  - [x] 완료 후 그 조각으로 세션을 더 태우면 소진과 `+N` 은 오르되 완료 제안이 다시 뜨지 않는다
  - [x] 항목을 초안에서 `×` 하고 확정 → 목록에서 사라지되 주간 총 소진은 그대로, **그 소진이 기타 행에 나타난다**
        — 폐기 결과는 확인했으나 **플래너의 `×` 가 아니라 드로어의 `보내주기` 로** 확인했다.
        당시 플래너에 다시 들어갈 길이 없었기 때문이며, 그 원인은 아래 발견 2 에서 고쳤다
  - [x] 예산 20 · 소진 23 상태에서 빨간색·경고 아이콘이 없다
  - [x] 예산을 **비운 채** 확정하면 게이지가 `<소진> / 미설정` + `예산을 정하면 …` 이고 `소진 / 0` 이 아니다
  - [x] 예산을 **0 으로** 확정하면 소진 숫자만 보이고 `/ 미설정` 도 `예산을 정하면 …` 도 **없다** (ux-spec §7 · A27)
  - [x] 항목을 20개 만들어도 게이지가 카드 하단에 남아 있다 — 목록만 스크롤한다 (§2)

#### 수동 검증 실측 기록 (2026-08-11, macOS)

빌드 산출물을 항목마다 빈 임시 프로필로 띄워 확인했다. 20뽀모 같은 수치는 타이머를 스무 번
돌릴 수 없으므로 `sessions` 에 직접 넣고, 그 밖의 조작은 전부 실물 화면을 눌러서 했다.

- **소진·게이지·도트는 함께 움직인다.** 조각 하나를 재생·완료하니 항목 도트가 `0/2 → 1/2`,
  게이지가 `0 → 1` 로 같은 렌더에서 올랐다. 조각을 완료하자 `할 일을 다 끝냈어요 — 이 할당도
  완료할까요?` 제안이 떴고, 그때까지 항목의 `completed_at` 은 비어 있었으며 `완료로 표시` 를
  누른 뒤에야 채워졌다. 그 뒤 세션을 더 태워도 제안은 다시 뜨지 않았다.
- **게이지 3종.** 예산 20·소진 23 은 `23 / 20 · +3 · 예산은 추정치예요 — 넘어가도 괜찮아요` 이고
  `--danger` 색과 아이콘이 **0건**이다(계산된 색을 직접 대조). 예산을 비우면 `3 / 미설정` 이고
  DB 의 `budget` 은 NULL, 예산 0 이면 화면에 `3` 만 남고 DB 는 0 이다 — 셋이 서로 다른 상태로
  구분된다.
- **항목 20개에서도 게이지는 카드 하단이다.** 게이지 하단 좌표와 카드 하단 좌표가 775px 로
  같고 목록만 스크롤한다(`scrollHeight > clientHeight`).
- **폐기 결과.** 세션이 붙은 항목을 보내주니 목록에서 빠지고 주간 총 소진은 `1` 그대로이며
  그 소진이 `기타 — 계획에 없던 집중` 으로 옮겨 갔다. `dropped_at` 이 찍혔고 삭제는 아니다.

##### 발견 1 — `항목 0 · 그 주 세션·조각은 있음` 상태에 CTA 가 없다 → 고침

[ux-spec §8](../features/week-plan/ux-spec.md) 의 빈 상태 표는 이 칸에도 `+ <주> 할당 잡기` 를
요구하는데, [WeekCard.tsx](../../src/renderer/features/week/WeekCard.tsx) 의 `EmptyState` 는
`unplanned-only` 에서 카피만 렌더하고 조기 반환한다. 계획 없이 집중 한 번만 해도 그 주에는
플래너로 들어갈 버튼이 사라진다.

> **고침 (PR #47).** 같은 CTA 를 그 상태에도 렌더한다. 문구와 좁은 여백은 그대로 뒀다 —
> 위에 기타 행이 이미 있어 "아무 일도 없었다"는 말이 되면 안 된다. 테스트는 버튼의 존재만이
> 아니라 **눌렀을 때 플래너가 열리는지**까지 본다.

##### 발견 2 — 활성 항목이 있으면 플래너로 돌아갈 길이 없다 → 고침

일반 뷰에서 플래너를 여는 컨트롤은 빈 상태 CTA 와 전부 완료 상태의 `수정` 둘뿐이다. 활성
항목이 하나라도 있으면 주간 카드의 버튼은 `드로어 열기`·`+ 오늘로` 만 남는다(실측: 카드 안
버튼 목록 전수 조회). 그런데 ux-spec §5.3 은 "이미 존재하는 항목을 수정하러 들어온 경우
초안 목록에 그 주의 기존 항목이 채워져 있고", §5.6 은 "주중 재수정으로 들어온 확정도 같은
경로다" 라고 **재진입을 전제**한다. 즉 명세가 있는 흐름에 진입점이 없다.

**예산이 이 결함의 가장 아픈 자리다.** 첫 확정에서 예산을 비워 두면 게이지가
`예산을 정하면 예산 대비 소진이 보여요` 라고 안내하는데, 그 상태에서 예산을 정할 방법이
화면에 없다. 이 체크리스트의 `초안에서 × 하고 확정` 을 드로어의 `보내주기` 로 대신한 이유도
같다 — 플래너를 다시 열 수 없었다.

> **고침 (PR #49).** 주간 카드 헤더 우측에 `수정` 버튼을 두되 **본문에 진입이 없을 때만**
> 그린다 — 진입은 한 번에 하나이고, 포커스를 돌려줄 자리도 하나로 남는다. ux-spec §2 가
> 그 위치를 명시하게 함께 고쳤다: 명세가 진입 이름만 정하고 자리를 비워 둔 것이 이 결함의
> 뿌리였으므로, 코드만 고치면 같은 구멍이 다시 생긴다.
>
> 실측으로 확인한 것 — 예산을 비운 채 확정한 뒤 `수정` 으로 들어가 `0 / 12` 로 만들고 같은
> 방문에서 항목도 추가했으며, 재현한 일요일 상태에서는 같은 버튼이 `다음 주 계획` 을 열어
> 새 항목이 다음 주에 쓰였다.

##### 발견 3 — 종료할 때 DB 를 먼저 닫아 앱이 죽지 않는다 → 고침

이 계획서의 항목은 아니지만 **같은 수동 검증 회차에서 드러났으므로** 여기 남긴다. 정리를
`before-quit` 에서 하는데 `app.quit()` 경로(Cmd+Q·자동화)는 그 뒤에 창의 close 핸들러가
실행되고, 그 핸들러가 종료 확인 조건을 판정하려고 타이머 스냅샷을 읽으면서 **이미 닫힌 DB**
를 친다. main 의 예외를 Electron 이 에러 박스로 띄우고, 그 모달이 종료를 붙잡는다.

> **고침 (PR #48).** 정리를 `will-quit` 으로 옮겼다. 종료 시간이 35초 타임아웃에서 143ms 로
> 떨어졌고, 보류돼 있던 **CI 의 E2E 가 되살아났다** — Linux 에서 6개 전부 16.6초에 통과한다.
> [ci.yml](../../.github/workflows/ci.yml) 이 "Linux 에서 앱이 안 죽는다"고 적어 둔 것이
> 플랫폼 문제가 아니라 이 종료 순서였다.
