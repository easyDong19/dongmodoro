...
- [x] **Step 4: 코어 루프 수동 검증** — 실물 앱을 눌러 확인한다. 자동 검증으로 대체하지 않는다.
  - [x] 새 DB 로 첫 실행(평일) — 배너가 **뜨지 않는다** (A1)
  - [x] `plan_lead_days` 를 그대로 두고 시계를 일요일로 옮겨 실행 — 배너가 뜨고 범위가 그 주 1개다 (A2)
  - [x] 배너를 무시한 채 타이머 시작·완료, 오늘 목록 pull, 플래너 열기가 전부 동작한다 (A4)
  - [x] 아무 항목도 건드리지 않고 확정 — 남은 항목 전체가 계획 대상 주에 생긴다 (A7)
  - [x] 확정 직후 배너가 **즉시** 사라진다 (A16)
  - [x] 한 항목을 `줄여서` 로 낮추고 확정 — 새 항목의 도트 수가 그 값이다 (A10)
  - [x] 한 항목을 `보내주기` 로 확정 — 원본이 그 주에 남아 있고 목록에서 빠진다 (A14)
  - [ ] 패널을 열어둔 채 오늘 목록에서 항목을 완료 처리한 뒤 확정 — **성공하고** 그 항목은 이월되지 않는다 (A22)
        — 앞의 절반(확정 성공)만 확인했다. 뒤의 절반은 **손으로 재현할 수 없다** (아래 기록 참조)
  - [x] 확정 후 주간 카드의 게이지·도트가 새 주 기준으로 다시 그려진다
  - [x] 3주 공백을 만든 DB 로 실행 — 정산 화면이 **1개**이고 항목만 3주분이다 (A3)
  - [x] 다크·라이트 양쪽에서 배너·패널을 본다 — **다크는 아직 한 번도 눈으로 확인된 적이 없다** (광원 실효 알파가 라이트의 2배다)

#### 수동 검증 실측 기록 (2026-08-11, macOS)

**시계를 옮기지 않고 계획일을 밀었다.** 정산 판정은 `weekOf(오늘 + plan_lead_days)` 와
워터마크의 비교이므로(services/review.ts), 화요일에 `plan_lead_days` 를 6 으로 두면
계획 대상 주가 다음 주가 되어 "일요일에 켠 앱"과 같은 상태가 된다. OS 시계를 건드리면
검증 대상이 아닌 것(로그 타임스탬프·다른 앱)까지 흔들린다.

- **A1·A2** — 빈 프로필 첫 실행에서 배너 0건이고 워터마크가 `2026-08-03` 으로 초기화됐다.
  계획일을 민 뒤 재실행하니 `이번 주 마감이 기다려요 · 다음 주로 넘어갈 3건` 이 뜨고
  범위는 그 주 하나였다.
- **A4** — 배너가 떠 있는 상태에서 타이머가 돌고 완료로 세션 1건이 기록됐고, 드로어에서
  오늘 조각을 만들어 오늘 목록에 넣었고, 배너는 그 사이 계속 떠 있었다.
- **A7·A10·A14** — 확정 뒤 DB 상태가 셋을 한 번에 말한다. 손대지 않은 `보고서 초안` 은
  계획 대상 주(`2026-08-17`)에 예상 3 으로 생겼고, `줄여서 1` 로 낮춘 `리팩터링` 은
  같은 주에 예상 **1** 로 생겼으며, `보내주기` 한 `회의 준비` 는 원래 주(`2026-08-10`)에
  `dropped_at` 이 찍힌 채 남고 다음 주에는 생기지 않았다.
- **A16** — 확정 직후 배너와 패널이 함께 사라지고 워터마크가 `2026-08-10` 으로 전진했다.
  카드는 폐기된 항목이 빠진 상태로 다시 그려지고 `다음 주로 2건 넘어갔어요` 가 떴다.
- **A3** — 과거 3주(7/13·7/20·7/27)에 항목을 심고 워터마크를 7/06 으로 되돌린 DB 로 실행하니
  배너가 `7/13 – 8/9` 한 줄이고 패널은 **1개**, 그 안에 6건이 출처 주 라벨과 함께 들어왔다.
  기록이 없는 주는 `1주 쉬었어요` 로 요약에만 나온다.
- **A22 의 절반은 손으로 재현할 수 없다.** 패널이 열리면 주간 카드 본문을 대체하므로
  (WeekCard.tsx), 패널을 연 채 도달할 수 있는 "다른 화면"은 오늘 목록뿐이다. 그런데 오늘
  목록의 완료는 **조각(task)** 의 완료이고 정산의 대기 목록은 **항목(week_item)** 의
  `completed_at` 으로 판정하므로(drizzle.ts `pendingPredicate`), 오늘 목록에서는 항목을
  완료시킬 수 없다. 확인된 것은 "그 사이 오늘 목록이 바뀌어도 확정이 성공한다"까지이고,
  "완료된 항목이 이월되지 않는다"는 `resolveDecisions` 의 유닛 테스트가 덮는다.
- **관찰 하나** — `줄여서` 를 고르면 스테퍼는 1 을 가리키는데 그 행의 도트는 계속 `0/2` 다.
  도트가 "남은 뽀모", 스테퍼가 "넘길 뽀모"라 서로 다른 값을 말하는 것이라 틀린 표시는
  아니지만, 결과를 미리 보여주는 자리에서 두 숫자가 갈린다. 결함으로 다루지 않고 기록만 남긴다.
