...
> **현재 상태: M3a 주간 계획 완료.**
> 플래너에서 이번 주 할당을 잡고, 항목에서 조각을 오늘로 가져오고, 재생을 눌러 집중
> 세션을 기록하면 그 뽀모가 주간 카드의 도트와 예산 게이지로 되돌아온다 — 주 → 오늘 →
> 실행 → 주 의 고리가 닫혔다.
> 정산·마일스톤·달력·반응형 셸·트레이는 아직 없다. 요일 정보에 매달린 기능 3종과 요일별
> 부하 그래프·기타 행 드릴다운은 M3b 정산이 값을 만들어 줄 때 살아난다.
> ([M3a 계획](docs/plans/2026-08-07-m3a-week-plan.md))
>
...
→ **M3b** 정산(다음) → **M4** 패키징(electron-builder, macOS 서명·공증, 트레이 세부).
세부 분할은 각 단계 착수 시점에 계획 문서로 추가된다.

M3a 가 **미룬** 것들이 M3b 와 함께 살아난다: "오늘이 무슨 요일인가" 정보에 매달린 기능
3종(요일 핍 4상태·오늘 배정 상단 정렬·플래너의 `다음 주`)은 `clock.now` 응답에 필드를
더하면 한 번에 풀리고, 요일별 부하 그래프는 정산이 `weekly_capacity` 편집 진입점을
만들어야 기준선이 생긴다. 기타 행 드릴다운은 그 둘과 독립이라 언제든 붙일 수 있다.
