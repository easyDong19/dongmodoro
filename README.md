...
> 마일스톤·달력·반응형 셸·트레이는 아직 없다. 뽀모 길이 편집 진입점과 요일별 부하
> 그래프는 pomo-baseline 이 가용량 편집을 만들어야 값이 생기고, 기타 행 드릴다운은
> 그 둘과 독립이라 언제든 붙일 수 있다.
...
4상태·오늘 배정 상단 정렬·플래너의 `다음 주`). 아직 값이 없어 미뤄져 있는 것은 셋이다:
**뽀모 길이 편집 진입점**은 pomo-baseline 이 편집 화면을 만들어야 갈 곳이 생기고, **요일별
부하 그래프**는 `weekly_capacity` 편집이 있어야 기준선이 생기며, **기타 행 드릴다운**은
그 둘과 독립이라 언제든 붙일 수 있다.
