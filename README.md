...
> **현재 상태: M4 패키징 완료 — 설치할 수 있는 앱이 나온다.**
...
> 효력을 갖는다. 여기에 **월 레이어**가 올라왔다 — 한 달의 결과물을 적고, 할당을 거기에
> 매달아 이번 주 진행을 범위 라벨과 함께 보며, 월 그리드에서 기록이 있는 날을 되짚는다.
> 두 카드는 달 이동을 공유한다. 반응형 셸·트레이는 여전히 없고, 첫 실행 온보딩과
> 요일별 부하 그래프, 기타 행 드릴다운도 남아 있다 ([PRODUCT.md](PRODUCT.md) 의 v1.0.0 정의).
> ([M3b 계획](docs/plans/2026-08-10-m3b-weekly-review.md) ·
> [베이스라인 편집 계획](docs/plans/2026-08-11-baseline-editing.md) ·
> [패키징 계획](docs/plans/2026-08-11-packaging-and-release.md) ·
> [월 레이어 계획](docs/plans/2026-08-12-month-layer.md))

## 설치 (macOS)

[Releases](https://github.com/easyDong19/dongmodoro/releases) 에서 `.dmg` 를 내려받아
`dongmodoro.app` 을 응용 프로그램 폴더로 옮긴다. Apple Silicon 전용이다.

**첫 실행은 막힌다.** 이 앱은 공증받지 않았기 때문이다 (근거:
[ADR-028](docs/architecture/decisions/adr-028-code-signing.md)). 앱이 깨진 것이 아니라
**macOS 가 내려받은 미공증 앱을 기본으로 차단하는 것**이며, 격리 속성을 지우면 열린다.

```bash
xattr -dr com.apple.quarantine /Applications/dongmodoro.app
```

시스템 설정 → `개인 정보 보호 및 보안` 에서 한 번 실행을 시도한 뒤 `확인 없이 열기` 를
눌러도 된다. 어느 쪽이든 **한 번만** 하면 된다.

> 서명은 유료 개발자 계정이 있어야 한다. 받는 사람과 만드는 사람이 같은 동안에는
> 그 비용을 지불하지 않기로 했고, 남에게 배포할 필요가 생기면 그 결정을 다시 본다.
>
...
> 상태에서 Electron 버전을 돌려주고, `package.json` 의 앱 버전은 **패키징된 앱에서만**
> 보인다. 개발 화면의 숫자가 `1.0.0` 이 아니라고 해서 버전이 안 올라간 것이 아니다.
...
→ **M3b** 정산(완료) → **M4** 패키징(완료 — `v1.0.0`).

그 다음은 버전으로 센다. **월 레이어(마일스톤 + 달력, 구현 완료)** → 반응형 셸·트레이.
이 넷은 원래 v1 안에 있었고, `1.0.0` 을 내면서 [PRODUCT.md](PRODUCT.md) 가 범위를 좁혔다 —
문서가 말하는 v1 과 태그가 말하는 v1 이 갈리지 않게 하기 위해서다.

로드맵이 오래 `1.1.0` 마일스톤 → `1.2.0` 달력 2단계로 세어 왔는데, **한 판으로 합쳤다.**
두 기능이 표시 대상 월과 달 이동 컨트롤 하나를 공유하므로(calendar-records R26 ·
milestones R19), 쪼개면 그 컨트롤을 임시로 한쪽에 얹었다가 넘겨주는 코드가 생긴다.
버전 번호는 태그를 다는 시점에 정한다 — 계획서 파일명이 버전이 아니라 내용인 것과 같은
이유다.

M3a 가 미뤘던 3종은 M3b 가 `clock.now` 에 요일 필드를 더하면서 함께 살아났다 (요일 핍
4상태·오늘 배정 상단 정렬·플래너의 `다음 주`). **뽀모 길이 편집 진입점**도 정산 패널의
`조정` 으로 열렸다. **MONTH 컬럼**이 실재하므로 반응형의 오버레이 토글도 이제 열 대상이
있다 — 그동안은 만들면 아무것도 열지 않는 버튼이었다. 남은 것은 셋이다: **첫 실행 온보딩**은 편집 경로 2개 중 나머지 하나이며
