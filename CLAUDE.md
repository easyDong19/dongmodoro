...
구현 계획서·설계 스펙은 **`docs/plans/` 에만** 쓰고(스킬 기본 경로 `docs/superpowers/` 금지),
**구현 PR 안에서 삭제한다** — 근거는 ADR·기능 문서로, 미룬 범위는 `docs/tmp/` 로 먼저 옮긴다.
절차: [docs/CLAUDE.md](docs/CLAUDE.md#계획을-끝낼-때--구현-pr-의-마지막-태스크).
...
- 태그는 release 브랜치에서 잘리므로 **노트 파일도 그 브랜치에 있어야** 한다. 순서로 푼다 — 노트를 main 에 머지한 **뒤** release 브랜치를 자르면 cherry-pick 할 것이 없다. 버전 범프 커밋은 그 반대로 **release 브랜치에만** 둔다 ([CONTRIBUTING §2](CONTRIBUTING.md)).
